...
export default function UploadForm({ user, onQuestionsGenerated }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:5678/webhook/interview-prep", {
        method: "POST",
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #f8f4ff 0%, #f0f7ff 50%, #f4fff8 100%)", fontFamily: "sans-serif" }}>
      ...
      {loading && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(255,255,255,0.92)", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", backdropFilter: "blur(4px)" }}>
          ...
          <div style={{ display: "flex", gap: "0.5rem" }}>
            {[0,1,2,3,4].map(i => (
              <div key={i} style={{ width: "10px", height: "10px", background: "#6c63ff", borderRadius: "50%", animation: `bounce 1s infinite`, animationDelay: `${i * 0.15}s`, opacity: 0.7 }} />
            ))}
          </div>
          <style>{`
      ...
      <nav style={{ padding: "1.5rem 2rem", display: "flex", justifyContent: "space-between", alignItems: "center", maxWidth: "700px", margin: "0 auto" }}>
        ...
        <button onClick={() => supabase.auth.signOut()} style={{ background: "none", border: "2px solid #e8e0ff", borderRadius: "20px", padding: "0.4rem 1rem", color: "#6c63ff", fontWeight: "700", cursor: "pointer", fontSize: "0.85rem" }}>
          Sign Out
        </button>
      </nav>
