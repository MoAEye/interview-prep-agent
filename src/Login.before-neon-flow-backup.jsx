/**
 * Backup of Login.jsx before integrating 21st.dev Neon Flow (WebGL tubes).
 * To revert: replace Login.jsx with this file (or copy its contents over Login.jsx).
 */
...
export default function Login({ onLogin, recruiterMode, initialSignUpMode, onConsumeInitialSignUp }) {
  ...
  return (
    ...
    >
      <style>{`
        @keyframes login-orb-a { 0%,100%{transform:translate(0,0) scale(1)} 50%{transform:translate(24px,-20px) scale(1.05)} }
        @keyframes login-orb-b { 0%,100%{transform:translate(0,0)} 50%{transform:translate(-30px,28px)} }
        @keyframes login-fade-up { from{opacity:0;transform:translateY(20px)} to{opacity:1;transform:translateY(0)} }
        @keyframes login-neon-shift { 0%,100%{background-position:0% 50%} 50%{background-position:100% 50%} }
        @keyframes spin { to { transform: rotate(360deg); } }
        .login-shell { animation: login-fade-up 0.55s ease both; }
        .login-neon-flow {
          border-radius: 1.25rem;
          padding: 1px;
          background: linear-gradient(125deg, #22d3ee, #8b5cf6, #e879f9, #06b6d4, #a855f7);
          background-size: 400% 400%;
          animation: login-neon-shift 10s ease infinite;
          box-shadow: 0 0 60px rgba(139, 92, 246, 0.15), 0 0 100px rgba(34, 211, 238, 0.08);
        }
      ...
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0 }}>
        <div
          style={{
            ...
            borderRadius: "50%",
            filter: "blur(4px)",
            animation: "login-orb-a 14s ease-in-out infinite",
          }}
        />
        <div
          style={{
            ...
            borderRadius: "50%",
            animation: "login-orb-b 18s ease-in-out infinite",
          }}
        />
        <div
          style={{
      ...
      <div className="login-shell" style={{ width: "100%", maxWidth: "440px", position: "relative", zIndex: 1 }}>
        {!isSupabaseConfigured && (
          <div
            style={{
        ...
        <p style={{ textAlign: "center", color: "#52525b", fontSize: "0.75rem", marginTop: "1.35rem" }}>Secured by Supabase · Your data stays private</p>
      </div>
    </div>
  );
}
