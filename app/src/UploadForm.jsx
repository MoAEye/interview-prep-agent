...
export default function UploadForm({ user, onQuestionsGenerated, onShowHistory, onExitDemo }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data?.error || res.statusText || "Request failed. Check the terminal where the app is running for details.");
        setLoading(false);
        return;
      }
      onQuestionsGenerated(data, jobTitle);
    } catch (e) {
      setError(e?.message || "Something went wrong. If the app was started with 'npm run start', check the terminal for errors.");
    }
    setLoading(false);
  };
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #f8f4ff 0%, #f0f7ff 50%, #f4fff8 100%)", fontFamily: "sans-serif" }}>
      ...
      <nav style={{ padding: "1.5rem 2rem", display: "flex", justifyContent: "space-between", alignItems: "center", maxWidth: "700px", margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: "0.75rem", alignItems: "center" }}>
          {isDemo && <span style={{ fontSize: "0.8rem", color: "#888", fontWeight: "600" }}>Demo — not saved</span>}
          {!isDemo && <button onClick={onShowHistory} style={{ background: "none", border: "2px solid #e8e0ff", borderRadius: "20px", padding: "0.4rem 1rem", color: "#6c63ff", fontWeight: "700", cursor: "pointer", fontSize: "0.85rem" }}>📋 History</button>}
          <button onClick={isDemo ? onExitDemo : () => supabase.auth.signOut()} style={{ background: "none", border: "2px solid #e8e0ff", borderRadius: "20px", padding: "0.4rem 1rem", color: "#6c63ff", fontWeight: "700", cursor: "pointer", fontSize: "0.85rem" }}>{isDemo ? "Exit demo" : "Sign Out"}</button>
        </div>
      </nav>
